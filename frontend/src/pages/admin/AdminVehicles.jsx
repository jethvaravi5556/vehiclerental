...
const AdminVehicles = () => {
  ...
  const vehicleStatsCalculated = useMemo(() => {
    ...
    const avgRating = vehicles.reduce((sum, v) => sum + (v.rating || 0), 0) / total || 0
  ...
  const toggleAvailability = async (vehicleId, currentStatus) => {
    ...
    const confirmed = window.confirm(
      `Are you sure you want to mark "${vehicle?.title}" as ${newStatus ? 'Available' : 'Booked'}?`
    )
    if (!confirmed) return
  ...
  const filteredAndSortedVehicles = useMemo(() => {
    ...
    filtered.sort((a, b) => {
      ...
      if (sortBy === "pricePerDay" || sortBy === "pricePerHour" || sortBy === "rating") {
  ...
  const handleDeleteVehicle = async (vehicleId, vehicleName) => {
    const confirmed = window.confirm(`Are you sure you want to delete "${vehicleName}"?`)
    if (!confirmed) return
  ...
  const exportVehicles = () => {
    const csvContent = "data:text/csv;charset=utf-8," 
      ...
      + filteredAndSortedVehicles.map(vehicle => 
          `"${vehicle.title}","${vehicle.brand}","${vehicle.type}",${vehicle.pricePerDay},${vehicle.pricePerHour},"${vehicle.location}",${vehicle.available ? 'Yes' : 'No'},${vehicle.rating || 0}`
        ).join("\n")
        ...
        <Card className="bg-gradient-to-r from-blue-600 to-purple-600 border-0 shadow-xl text-white overflow-hidden">
          <div className="p-6">
            <div className="flex items-center justify-between mb-6">
              <div>
                <h2 className="text-2xl font-bold mb-2">Fleet Analytics</h2>
                <p className="text-blue-100">Monitor and manage your vehicle inventory</p>
              </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
              {[
                { label: "Total Vehicles", value: vehicleStatsCalculated.total, icon: Car, change: "+5%" },
                { label: "Available", value: vehicleStatsCalculated.available, icon: CheckCircle, change: "+3%" },
                { label: "Booked", value: vehicleStatsCalculated.booked, icon: Clock, change: "+12%" },
                { label: "Avg Rating", value: vehicleStatsCalculated.avgRating, icon: Star, change: "+0.2" }
              ].map((stat, i) => (
                <motion.div
                  key={stat.label}
                  initial={{ opacity: 0, y: 20 }}
            ...
            <div className="flex flex-col lg:flex-row gap-4">
              ...
              <div className="flex gap-3">
                <select
                  ...
                  className="px-4 py-3 border border-gray-200 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-transparent bg-white/70"
                >
                  ...
                  <Card className="bg-white/80 backdrop-blur-sm border-0 shadow-lg hover:shadow-xl transition-all duration-300 overflow-hidden">
                    ...
                    <div className="relative h-48 bg-gradient-to-br from-gray-100 to-gray-200 overflow-hidden">
                      ...
                      <div className="absolute top-3 left-3">
                        <span className={`inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium ${getStatusColor(vehicle.available)}`}>
                      ...
                      <div className="absolute bottom-3 right-3">
                        <Button
                          ...
                          title={`Toggle to ${vehicle.available ? 'Booked' : 'Available'}`}
                        >
                    ...
                    <div className="p-4">
                      ...
                      <div className="flex space-x-2">
                        ...
                        <Button
                          variant="outline"
                          size="sm"
                          onClick={() => handleDeleteVehicle(vehicle._id, vehicle.title)}
                          disabled={processing === vehicle._id}
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-3">
                  <div>
                    ...
                    <p className="text-gray-900 font-medium">{selectedVehicle.title}</p>
                  </div>
                  <div>
                ...
                <div className="space-y-3">
                  <div>
                    ...
                    <p className="text-gray-900 font-medium">₹{selectedVehicle.pricePerDay}</p>
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-gray-700">Hourly Price</label>
                    <p className="text-gray-900 font-medium">₹{selectedVehicle.pricePerHour}</p>
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-gray-700">Rating</label>
                    <div className="flex items-center">
                      <Star className="h-4 w-4 text-yellow-400 fill-current mr-1" />
                      <span className="text-gray-900">{selectedVehicle.rating || 'No rating'}</span>
                    </div>
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-gray-700">Status</label>
                    <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${getStatusColor(selectedVehicle.available)}`}>
                ...
                <div>
                  ...
                  <div className="grid grid-cols-2 gap-4 p-4 bg-gray-50 rounded-lg">
                    <div>
                      ...
                      <span className="ml-2 text-gray-900">{selectedVehicle.specs.transmission || 'N/A'}</span>
                    </div>
                    <div>
                      <span className="text-sm font-medium text-gray-700">Seats:</span>
                      <span className="ml-2 text-gray-900">{selectedVehicle.specs.seats || 'N/A'}</span>
                    </div>
                    <div>
                      <span className="text-sm font-medium text-gray-700">Fuel:</span>
                      <span className="ml-2 text-gray-900">{selectedVehicle.specs.fuel || 'N/A'}</span>
                    </div>
                    {selectedVehicle.specs.features && selectedVehicle.specs.features.length > 0 && (
                      <div className="col-span-2">
                        <span className="text-sm font-medium text-gray-700">Features:</span>
                        <div className="mt-1 flex flex-wrap gap-1">
                          {selectedVehicle.specs.features.map((feature, i) => (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) => setCreateForm(prev => ({ ...prev, pricePerDay: e.target.value }))}
                  className="w-full px-3 py-2 border border-gray-200 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                  placeholder="1500"
                />
              </div>
              <div>
                ...
                <input
                  ...
                  onChange={(e) => setCreateForm(prev => ({ ...prev, pricePerHour: e.target.value }))}
                  className="w-full px-3 py-2 border border-gray-200 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                  placeholder="200"
                />
              </div>
          ...
          {editForm && (
            <div className="space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => setEditForm(prev => ({ ...prev, pricePerHour: e.target.value }))}
                    className="w-full px-3 py-2 border border-gray-200 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                  />
                </div>
              </div>
              ...
              <div className="p-4 bg-blue-50 rounded-lg">
                <div className="flex items-center justify-between">
                  <div>
                    <h4 className="font-medium text-gray-900">Vehicle Availability</h4>
